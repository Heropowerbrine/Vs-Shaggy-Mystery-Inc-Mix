...
class Note extends FlxSprite
{
	...
	public var opponentHit:Bool = false;
	...
	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?inEditor:Bool = false)
	{
		...
		if (isSustainNote && prevNote != null)
		{
			...
			if (prevNote.isSustainNote)
			{
				...
				prevNote.scale.y = (Conductor.stepCrochet * 0.45 * PlayState.SONG.speed) / prevNote.frameHeight;
	...
	override function update(elapsed:Float)
	{
		...
		if (mustPress)
		{
			...
			if (noteType == 4) //death notes
			{
				if (strumTime > Conductor.songPosition - 30
					&& strumTime < Conductor.songPosition + 30) //30 should be same as og
					canBeHit = true;
				else
					canBeHit = false;
			}
