...
class MansionDebris extends FlxSprite
{
	...
	var time:Float = 0;
	public function new(sX:Float, sY:Float, debName:String, scroll:Float, tFactor:Float, tDelay:Float, posFactor:Float, scale:Float = 1.0)
	{
		sx = sX;
		sy = sY;
		x = sx;
		...
		animation.play('c');
		loadGraphic(Paths.image("GODBG/" + debName));
		scrollFactor.set(sc, sc);
		antialiasing = true;
		setGraphicSize(Std.int(frameWidth * (sc / 0.75) * scale));
